...
function NavLink({ title, icon, label, href, closeNav, subLink = false }) {
  const { checkActiveNav, checkIfActiveSubNav } = useCheckActiveNav()
  return (
    <Link
      to={href}
      onClick={closeNav}
      className={cn(
        buttonVariants({
          variant: checkActiveNav(href)
            ? 'default'
            : checkIfActiveSubNav(href)
              ? 'secondary'
              : 'ghost',
          size: 'sm',
        }),
        'ml-1 h-12 justify-start text-wrap rounded-l-sm px-6',
...
function NavLinkDropdown({
  ...
  const isChildActive = !!sub?.find((s) => checkActiveNav(s.href, parentHref))
    ...
    <Collapsible defaultOpen={isChildActive}>
      <CollapsibleTrigger
        className={cn(
          ...
          'group ml-1 h-12 w-full justify-start rounded-sm px-6'
        )}
      >
        <div className='mr-2'>{icon}</div>
        {title}
...
function NavLinkIconDropdown({ title, icon, href: parentHref, label, sub }) {
  const { checkActiveNav } = useCheckActiveNav()
